...
function buildRegionButtons() {
  regionOrder.forEach((key) => {
    ...
    button.textContent = data.name.replace(/^(ال)?منطقة /, "");
...
const nationalBtn = document.querySelector("#national-belonging-btn");
const nationalModal = document.querySelector("#national-modal");
let unityTimeouts = [];
let lastFocusedBeforeNationalModal = null;

function clearUnityTimeouts() {
  unityTimeouts.forEach((id) => window.clearTimeout(id));
  unityTimeouts = [];
}

function resetUnityClasses() {
  document.querySelectorAll(".region-shape").forEach((shape) => {
    shape.classList.remove("unity-glow", "unity-united");
  });
}
...
function playUnityAnimation() {
  ...
  regionOrder.forEach((key, index) => {
    const shape = document.querySelector(`.region-shape[data-key="${key}"]`);
    if (!shape) return;
    const onId = window.setTimeout(() => shape.classList.add("unity-glow"), index * stagger);
    const offId = window.setTimeout(() => shape.classList.remove("unity-glow"), index * stagger + 420);
    unityTimeouts.push(onId, offId);
  });
  const uniteDelay = regionOrder.length * stagger + 300;
  const uniteId = window.setTimeout(() => {
    document.querySelectorAll(".region-shape").forEach((shape) => shape.classList.add("unity-united"));
  }, uniteDelay);
  const revertId = window.setTimeout(resetUnityClasses, uniteDelay + 1100);
  unityTimeouts.push(uniteId, revertId);
}

function openNationalModal() {
  if (!nationalModal) return;
  lastFocusedBeforeNationalModal = document.activeElement;
  nationalModal.classList.add("is-open");
  nationalModal.setAttribute("aria-hidden", "false");
  document.body.classList.add("modal-open");
  nationalModal.querySelector(".national-modal-close").focus({ preventScroll: true });
  playUnityAnimation();
}

function closeNationalModal() {
  if (!nationalModal) return;
  nationalModal.classList.remove("is-open");
  nationalModal.setAttribute("aria-hidden", "true");
  document.body.classList.remove("modal-open");
  clearUnityTimeouts();
  resetUnityClasses();
  if (lastFocusedBeforeNationalModal instanceof HTMLElement) {
    lastFocusedBeforeNationalModal.focus({ preventScroll: true });
  }
}

if (nationalBtn) {
  nationalBtn.addEventListener("click", openNationalModal);
}

if (nationalModal) {
  nationalModal.querySelectorAll("[data-national-close]").forEach((el) => {
    el.addEventListener("click", closeNationalModal);
  });
  document.addEventListener("keydown", (event) => {
    if (event.key === "Escape" && nationalModal.classList.contains("is-open")) {
      closeNationalModal();
    }
  });
}
